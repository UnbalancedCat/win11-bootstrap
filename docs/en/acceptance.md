...
5. From an ordinary, non-administrator Windows PowerShell, run `tests\acceptance\Invoke-SelfElevationProbe.ps1 -CandidateRoot <expanded-candidate>` from the matching toolkit commit. Require exactly one UAC prompt, no provider installation, and exit 0. Never bypass a failure by starting the candidate as administrator.
