...
6. Then preflight `clash-verge-rev`, `chrome`, `git`, `codex-desktop`, `realvnc-server`, and `nomachine`, and repeat the same command once. A runtime defect requires a separate PR and a new candidate. Freeze only after package IDs, Store identity, pinned versions, and manual-only sources are rechecked.
