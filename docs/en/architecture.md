...
The entry point never interpolates user paths into PowerShell source or relies on temporary process environment variables crossing UAC. Before UAC it resolves configuration, bounds option sizes, fixes runtime filenames, lengths, and SHA-256 values in the loader manifest, and creates random one-use request and result pipes. Both pipes disable ACL inheritance, explicitly deny the Network SID, and allow only the current user SID and SYSTEM. The parent sends the envelope only to the exact child PID returned by `Start-Process -Verb RunAs`; the child verifies the exact parent PID through the operating-system API. The envelope must match the command-bound length, SHA-256, invocation ID, and property set. After cleanup, the same child returns a fixed 41-byte frame containing the invocation ID and only `0`, `10`, `20`, `30`, or `64`; the parent binds that result to the exact child PID and treats it as authoritative instead of relying on ShellExecute's observation of a nonzero process exit. A timeout, early child exit, trailing data, or any PID, digest, shape, invocation, or status mismatch fails closed, with no fallback to command-line, disk, or cross-UAC environment transport.
