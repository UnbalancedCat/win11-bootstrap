...
3. `WhatIf` 保持普通权限；真实执行先把配置解析为规范选项、对运行文件生成 SHA-256 清单并建立一次性本机命名管道，然后只触发一次 UAC。命令行只携带随机管道名、进程绑定信息、信封长度和摘要，不携带选项或 loader。
4. 提权后的最小客户端与普通权限父进程双向校验 PID，并校验信封的精确长度、SHA-256、版本和字段集合。随后经验证的 loader 把清单文件复制到受限的 ProgramData 随机快照，逐项复验后从快照启动实际入口；原始配置文件不会在权限边界另一侧重新读取。
...
入口不会把用户路径拼成 PowerShell 命令，也不依赖临时进程环境变量跨越 UAC。它在 UAC 前解析配置、限制参数大小，把运行所需文件名、长度和 SHA-256 固定到 loader 清单，并建立随机的一次性本机命名管道。管道 ACL 关闭继承、显式拒绝 Network SID 且只允许当前用户 SID 和 SYSTEM。父进程只向 `Start-Process -Verb RunAs` 返回的精确子 PID 发送信封，子进程也通过系统 API 验证精确父 PID。信封必须满足命令行中固定的长度、SHA-256、调用 ID 和字段集合，超时、PID/摘要/结构不匹配均失败关闭，不回退到命令行、磁盘或跨 UAC 环境传参。
